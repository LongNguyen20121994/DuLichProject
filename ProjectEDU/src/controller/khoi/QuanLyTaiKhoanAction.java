...
public class QuanLyTaiKhoanAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(listSoCMND != null && listSoCMND.length > 0){
			if(btnSua != null){
				btnSua = null;
				if (account.equals("1")) {
					new ThiSinhBO().kichHoatTaiKhoan(listSoCMND, false, true);
				}
				if (account.equals("2")) {
					new GiaoVienBO().kichHoatTaiKhoan(listSoCMND, false, true);
				}
				if (account.equals("3")) {
					new GiangVienBO().kichHoatTaiKhoan(listSoCMND, false, true);
				}
				if (account.equals("4")) {
					new QuanLyCumThiBO().kichHoatTaiKhoan(listSoCMND, false, true);
				}
				if (account.equals("5")) {
					new QuanTriVienBO().kichHoatTaiKhoan(listSoCMND, false, true);
				}
			} else {
				if(btnXoa != null){
					btnXoa = null;
	...
	public String showQuanLyTaiKhoan(){
		info = new LoginAction().checkLogin("5");
		if(info != null) {
			if(info.getTieuDe() == null){
				return "login";
		...
		if (account.equals("1")) {
			listThiSinh = new ThiSinhBO().getListThiSinhByTrangThai(true, true);
		}
		if (account.equals("2")) {
			listGiaoVien = new GiaoVienBO().getListGiaoVienByTrangThai(true, true);
		}
		if (account.equals("3")) {
			listGiangVien = new GiangVienBO().getListGiangVienByTrangThai(true, true);
		}
		if (account.equals("4")) {
			listQuanLyCumThi = new QuanLyCumThiBO().getListQuanLyCumThiByTrangThai(true, true);
		}
		if (account.equals("5")) {
			listQuanTriVien = new QuanTriVienBO().getListQuanTriVienByTrangThai(true,true);
		}

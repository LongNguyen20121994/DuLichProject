...
public class GiaoVienDAO {
        ...
        try {
        	...
        	for(String tmp : listSoCMND){
	    		...
	    		if(check > 0){
	    			...
	    			if(rsEmail.next()){
	    				String tieuDe = (trangThai && !logined?"Kích hoạt" : trangThai && logined?"Mở khóa" : "Khóa") + " tài khoản giáo viên Trường Trung học phổ thông";
	    				...
	    				if(trangThai && !logined) {
		    				noiDung.append("<div style='padding:2% 3.5%;'><p>Bạn đã thực hiện đăng ký tài khoản giáo viên trường \""+rsEmail.getString(4));
		    				...
		    				noiDung.append("<ul><li>Đăng ký dự thi cho thí sinh trong trường</li><li>Cập nhật hồ sơ đăng ký dự thi của thí sinh trong trường</li>");
		    				noiDung.append("<li>Tải về danh sách thí sinh đăng ký thành công (kèm mật khẩu)</li></ul>");
	...
	public boolean quenMatKhau(String soCMND, String email) {
        ...
        try {
    		...
    		ResultSet rsEmail = stmtEmail.executeQuery();
    		if(rsEmail.next()){
				String tieuDe = "Quên mật khẩu tài khoản giáo viên";
				StringBuilder noiDung = new StringBuilder("<div style='border:5px inset #CCC;'>");
				noiDung.append("<h2 style='background-color:#4d90fe;padding:1.5% 3%; margin:2px;'>Xin chào " + rsEmail.getString(1) + "</h2>");
